...
data_name = "S14_mod_2_gauss_kernel"
add <- function(x) Reduce("+", x)

S14_B1 <- read_csv("data/S14-B1.csv")
data_1 = S14_B1 %>% as.matrix
S14_B2 <- read_csv("data/S14-B2.csv")
data_2 = S14_B2 %>% as.matrix
S14_B3 <- read_csv("data/S14-B3.csv")
data_3 = S14_B3 %>% as.matrix
...
sub_griddf = griddf[sample.int(100*100, size = 1000),] %>% as.matrix
...
var_function = function(x) 0.0001  #0.001x # 2*abs(x) optional: make heteroscedastic
...
for (i in 1:n_exp) {
  ...
  for (b in 1:budget) {
    ...
    y = apply(design,1, obj_fun)
    design_fitted = cbind(design,y)
    res_mbo = mbo(fun = obj_fun, design = design_fitted, control = ctrl, learner = lrn, show.info = F)
