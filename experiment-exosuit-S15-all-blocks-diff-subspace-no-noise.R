...
var_function = function(x)  0.001*x # 2*abs(x) optional: make heteroscedastic
...
utility = function(x){
  ...
  #- rnorm(1, mean = mean, sd = sd) # noisy evaluation
  mean
}
...
parameter_set = makeNumericParamSet(
  len = 2, id = "x", 
  lower = 0, upper = 30,
  vector = TRUE)
